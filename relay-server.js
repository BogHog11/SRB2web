  ...
  ws.on("message", (data) => {
    ...
    if (json.method == "connect") {
      ...
      if (netgames[id]) {
        ...
        ws.send(JSON.stringify({ method: "connected" })); // Optional hack
      } else {
        ws.send(JSON.stringify({ method: "error", message: "Netgame not found" }));
      }
    }
    ...
    if (json.method == "listen") {
      ...
      currentNetgame = {
        ...
        open: function (otherWs) {
          ...
          console.log("Client joined netgame: " + netId + " as ID: " + customId);
          // Notify the server about the join
          ws.send(JSON.stringify({ method: "join", id: customId, ip: otherWs._relayIP }));
        },
